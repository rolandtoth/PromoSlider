...
reverseDirection||false;d.rootDir=d.rootDir||"";d.running=null;d.showOnUrl=d.showOnUrl||null;d.showProbability=d.
showProbability||1;d.slideDuration=d.slideDuration>=0?d.slideDuration:1;d.slides=d.slides||null;d.startDate=d.startDate
||null;d.state="lightbox";d.vertical=d.vertical||null;d.waitAnimationFinish=d.waitAnimationFinish||null;d.width=d.width
||null;if(!d.slides)return false;getUrlOptions(d);f=!!d.forceOnUrl&&findInArray(d.forceOnUrl,r);c=!!d.deleteCookieOnUrl
&&findInArray(d.deleteCookieOnUrl,r);h=(function(a){if(!a||isArray(a)&&!a.length)return false;if(!!a&&!findInArray(a,r))
return true})(d.showOnUrl);g=!!d.hideOnUrl&&findInArray(d.hideOnUrl,r);if(!f){if(d.showProbability<1&&!checkRandomSeed(d
.showProbability))return false;if(d.startDate)if(e<Math.round(new Date(d.startDate).getTime()/1000))return false;if(d.
endDate)if(e>Math.round(new Date(d.endDate).getTime()/1000))return false;if(c)cookie.eraseCookie(d.promoID);if(h)
return false;if(g)return false;if(d.frequency){if(cookie.readCookie(d.promoID))return false;if(!c)cookie.createCookie(d.
promoID,"1",d.frequency)}}if(!findInArray(d.easing,["linear","ease-in","ease-in-out"]))d.easing="ease-in-out";if(!
...
seekAnimSpeed=d.slideDuration/d.slideNum*3/2;d.current=0};b.buildPromo=function(){var m,c,i,h,p,g,k,l,w,j,f,o,a;b.promo=
{overlay:makeElement("div",{"class":"promoOverlay"}),container:makeElement("div",{"class":"promoContainer"}),content:
makeElement("div",{"class":"promoContent"}),slidesWrap:makeElement("div",{"class":"promoSlidesWrap"}),slides:makeElement
("ul",{"class":"promoSlides"}),close:makeElement("a",{"class":"promoClose",href:"#"},d.closeButtonText),actionButtons:
